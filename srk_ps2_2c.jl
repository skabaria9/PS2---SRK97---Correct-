...
#K_T
McClureSlope_eq = 1.04*10^-3*1/60; #mM*min
K_T = 50; #mM, BIOID: 107327
#https://bionumbers.hms.harvard.edu/bionumber.aspx?id=107327&ver=2&trm=e+coli++ribosome+binding+constant&org=
...
#Convert everything to umol/gDW
...
I_dW_graph = zeros(length(tSim)) #set default all zeros in I
for i in 1:length(tSim)
      if tSim[i] < 60
            #print(tSim[i])
            I_dW_graph[i] = convertfactor(10)
      else
            I_dW_graph[i] = 0
      end
end

#in umol/gDW Plotting
figure(1)
plot(tSim,p1_gDW,color="purple")
plot(tSim,p2_gDW,color="blue")
plot(tSim,p3_gDW,color="red")
plot(tSim,I_dW_graph,color="green")
xlabel("time (min)")
